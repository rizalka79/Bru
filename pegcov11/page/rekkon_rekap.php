<?php
  ...
  $sql_ming = $koneksi->query("SELECT DISTINCT konming_tahun, konming_bulan FROM tb_konming ORDER BY konming_tahun ASC, konming_bulan ASC, konming_minggu ASC");
  while ($data_ming=$sql_ming->fetch_assoc()) {
    $konming_tahun    = $data_ming['konming_tahun'];
    $konming_bulan    = $data_ming['konming_bulan'];
    $konbul_isman     = 0;
    $konbul_rujuk     = 0;
    $konbul_sembuh    = 0;
    $konbul_meninggal = 0;

    $sql_ming2 = $koneksi->query("SELECT * FROM tb_konming WHERE konming_tahun ='$konming_tahun' AND  konming_bulan ='$konming_bulan' ORDER BY konming_tahun DESC, konming_bulan DESC, konming_minggu DESC");
    while ($data_ming2=$sql_ming2->fetch_assoc()) {
      $konbul_isman   = $konbul_isman + $data_ming2['konming_isman'];
      $konbul_rujuk   = $konbul_rujuk + $data_ming2['konming_rujuk'];
      $konbul_sembuh   = $konbul_sembuh + $data_ming2['konming_sembuh'];
      $konbul_meninggal   = $konbul_meninggal + $data_ming2['konming_meninggal'];
    }
    $konbul_total   = $konbul_isman+$konbul_rujuk+$konbul_sembuh+$konbul_meninggal;
